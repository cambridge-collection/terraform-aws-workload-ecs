...
resource "aws_autoscaling_attachment" "automatic_attachment" {
  count = var.allow_public_access && var.ecs_network_mode != "awsvpc" ? 1 : 0 # NOTE autoscaling attachments only support instance targets

  autoscaling_group_name = var.asg_name
  lb_target_group_arn    = aws_lb_target_group.this.0.arn
}
