...
resource "aws_vpc_security_group_egress_rule" "alb_egress_to_asg" {
  count = var.allow_public_access ? length(var.alb_target_group_settings) : 0

  ip_protocol                  = "tcp"
  description                  = "ALB Egress on port ${var.alb_target_group_settings[count.index].port} for ${var.alb_target_group_settings[count.index].name}"
  security_group_id            = var.alb_security_group_id
  referenced_security_group_id = var.asg_security_group_id
  from_port                    = var.alb_target_group_settings[count.index].port
  to_port                      = var.alb_target_group_settings[count.index].port

  tags = {
    Name = format("alb-egress-to-asg-%s", var.alb_target_group_settings[count.index].port)
  }
}
...
resource "aws_vpc_security_group_ingress_rule" "asg_ingress_from_alb" {
  ...
  ip_protocol                  = "tcp"
  description                  = "ASG Ingress on port ${var.alb_target_group_settings[count.index].port} for ${var.alb_target_group_settings[count.index].name}"
  ...
  tags = {
    Name = format("asg-ingress-from-alb-%s", var.alb_target_group_settings[count.index].port)
  }
}
...
resource "aws_vpc_security_group_egress_rule" "private_access_egress" {
  count = var.allow_private_access && var.update_asg_security_group_to_access_service ? length(var.alb_target_group_settings) : 0

  ip_protocol                  = "tcp"
  description                  = "Egress on port ${var.alb_target_group_settings[count.index].port} to ${var.ingress_security_group_id}"
  security_group_id            = var.asg_security_group_id
  ...
  tags = {
    Name = format("asg-egress-private-access-%s", var.alb_target_group_settings[count.index].port)
  }
}
