locals {
  ...
  datasync_s3_bucket_arn = length(var.datasync_s3_source_bucket_name) > 0 ? data.aws_s3_bucket.datasync.0.arn : local.s3_task_execution_bucket_arn
}

data "aws_s3_bucket" "datasync" {
  count = length(var.datasync_s3_source_bucket_name) > 0 ? 1 : 0

  bucket = var.datasync_s3_source_bucket_name
}
